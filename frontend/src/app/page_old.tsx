    ...
    return (
        ...
        <div className="mb-6">
                <h3 className="text-lg font-medium mb-4">
                    {selectedPieCategory ? `${selectedPieCategory} Trend` : 'Amount Trend'}
                </h3>
                        ...
                        <BarChart data={barChartData}>
                            ...
                            {subcategories.map((subcat, index) => (
                              <Bar 
                                  ...
                                  name={subcat}
                                  onClick={(data: any) => {
                                      if (!data || !data.payload) return; // TODO: ask chat, it used to look for activePayloadData                                      
                                      // Find matching transaction for this date
            ...
            <div>
                ...
                <div className="divide-y divide-gray-200">
                  {transactions
                      ...
                      .map((transaction) => {
                          // Create a unique key combining multiple transaction properties
                        const uniqueKey = `${ transaction.id }`; // `${transaction.date}-${transaction.amount}-${transaction.description.replace(/\s+/g, '')}-${transaction.memo || ''}-${transaction.checkNumber || ''}`;
                          return (
                              <div 
                                  key={uniqueKey}
